...
mod_upload_ui <- function(id) {
  ...
  tagList(
    ...
    selectInput(
      ns("name_convention"),
      "Naming convention",
      choices = NULL   # let server fully control it
    ),
    br(),
    fluidRow(
...
# allows to upload custom metadata file
    ...
    observeEvent(input$load_preview, {
      ...
      if (!is.null(chosen_meta)) {

        possible_names <- grep("^name_", names(chosen_meta), value = TRUE)
      ...
      if (!is.null(mm_obj) && !is.null(input$name_convention)) {
        mm_obj <- tryCatch({
          change_naming_convention(mm_obj, input$name_convention)
        }, error = function(e) {
          message("Naming convention failed: ", e$message)
          mm_obj
        })
      }
