...
mod_upload_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    path_meta <- system.file("extdata", "dt_meta.csv", package = "metamet")

    if (path_meta == "") {
      stop("dt_meta.csv not found in package extdata/")
    }

    #builtin_dt_meta <- readxl::read_excel(path_meta, sheet = "dt_meta") |>
    builtin_dt_meta <- readr::read_csv(path_meta, show_col_types = FALSE) |>
      as.data.frame()
